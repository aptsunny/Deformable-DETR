...
@torch.no_grad()
def check_forward_equal_with_pytorch_float_onnx_remove_split():
    value = torch.rand(N, S, M, D).cuda() * 0.01
    sampling_locations = torch.rand(N, Lq, M, L, P, 2).cuda()
    attention_weights = torch.rand(N, Lq, M, L, P).cuda() + 1e-5
    attention_weights /= attention_weights.sum(-1, keepdim=True).sum(-2, keepdim=True)
    ...
    model = MSDeformAttnCorePyTorch()
    model.eval()
    torch.onnx.export(model, (value, shapes, sampling_locations, attention_weights), 'ms_deform_attn_core_pytorch.onnx', opset_version=20)
    # 加载 ONNX 模型并运行
    sess = onnxruntime.InferenceSession('ms_deform_attn_core_pytorch.onnx')
    inputs = {
    ...
    # value_list = value.double().split([H_ * W_ for H_, W_ in shapes], dim=1)
    value_list = value.split([H_ * W_ for H_, W_ in shapes], dim=1)
    ...
    output_pytorch_v1 = ms_deform_attn_core_pytorch_kk(sampling_locations, attention_weights, value_list_f1, value_list_f2, value_list_f3, value_list_f4).detach().cpu()

    # 第五 拆分输入再进行onnx导出 实现
    model = MSDeformAttnCorePyTorch_Split()
    model.eval()
    torch.onnx.export(model, (sampling_locations, attention_weights, value_list_f1, value_list_f2, value_list_f3, value_list_f4), 'ms_deform_attn_core_pytorch_kk.onnx', opset_version=20)
    # 加载 ONNX 模型并运行
    sess = onnxruntime.InferenceSession('ms_deform_attn_core_pytorch_kk.onnx')
    inputs = {
        'sampling_locations': sampling_locations.cpu().numpy(),
    ...
    results = compare_outputs_include_mean(outputs, names)

    # simpy
    # import onnxsim, onnx
    # file = 'ms_deform_attn_core_pytorch_kk.onnx'
    # model_onnx = onnx.load(file)
    # onnx.checker.check_model(model_onnx)
    # print(f"Simplifying with onnx-simplifier {onnxsim.__version__}...")
    # model_onnx, check = onnxsim.simplify(model_onnx)
    # assert check, "assert check failed"
    # onnx.save(model_onnx, 'ms_deform_attn_core_pytorch_sim.onnx')


    # 创建表格
    df = pd.DataFrame(results, columns=['Output 1', 'Output 2', 'Equal', 'Max Abs Error', 'Max Rel Error', 'Mean Abs Error', 'Mean Rel Error'])
    print(df)


@torch.no_grad()
def check_forward_equal_with_pytorch_float_onnx_remove_split_random():
    N, M, D = 2, 8, 32
    shapes = torch.as_tensor([(48, 120), (24, 60), (12, 30), (6, 15)], dtype=torch.long).cuda()
    Lq, L, P = 356, 4, 4
    level_start_index = torch.cat((shapes.new_zeros((1, )), shapes.prod(1).cumsum(0)[:-1]))
    ...
    for _ in range(100):
        # 随机生成输入
        # value = torch.rand(N, S, M, D).cuda() * 0.01
        value = torch.randn(N, S, M, D).cuda()
        sampling_locations = torch.rand(N, Lq, M, L, P, 2).cuda()
        attention_weights = torch.rand(N, Lq, M, L, P).cuda() + 1e-5
        attention_weights /= attention_weights.sum(-1, keepdim=True).sum(-2, keepdim=True)
        ...
        # 第四 拆分输入实现
        value_list = value.split([H_ * W_ for H_, W_ in shapes], dim=1)
        value_list_f1 = value_list[0].flatten(2).transpose(1, 2).reshape(N*M, D, shapes[0][0], shapes[0][1])
        value_list_f2 = value_list[1].flatten(2).transpose(1, 2).reshape(N*M, D, shapes[1][0], shapes[1][1])
        value_list_f3 = value_list[2].flatten(2).transpose(1, 2).reshape(N*M, D, shapes[2][0], shapes[2][1])
        value_list_f4 = value_list[3].flatten(2).transpose(1, 2).reshape(N*M, D, shapes[3][0], shapes[3][1])
        output_pytorch_v1 = ms_deform_attn_core_pytorch_kk(sampling_locations, attention_weights, value_list_f1, value_list_f2, value_list_f3, value_list_f4).detach().cpu()
        ...
        outputs = [output_pytorch, output_cuda, torch.from_numpy(ort_output), output_pytorch_v1, torch.from_numpy(ort_output_v1)]
        names = ['PyTorch', 'CUDA', 'ONNX', 'PyTorch Split', 'ONNX Split']
        results = compare_outputs_include_mean_random(outputs, names)
        all_results.append(results)

    # 计算平均结果
    avg_results = []
    for i in range(len(results)):
        avg_result = [results[i][0], results[i][1], results[i][2]]
        for j in range(3, len(results[i])):
            avg_value = np.mean([all_results[k][i][j] for k in range(100)])
            avg_result.append(avg_value)
        avg_results.append(avg_result)

    # 创建表格
    df = pd.DataFrame(avg_results, columns=['Output 1', 'Output 2', 'Equal', 'Max Abs Error', 'Max Rel Error', 'Mean Abs Error', 'Mean Rel Error'])
    print(df)


def compare_outputs_include_mean_random(outputs, names):
    """
    比较多个输出的差异
    :param outputs: 输出列表
    :param names: 输出名称列表
    ...
    for i in range(num_outputs):
        for j in range(i + 1, num_outputs):
            ...
            rel_err = abs_err / output_i.abs().clamp(min=1e-8)  # 避免除以0
            ...
            results.append([name_i, name_j, is_equal, max_abs_err, max_rel_err, mean_abs_err, mean_rel_err])
    return results


def compare_outputs_include_mean(outputs, names):
    """
    比较多个输出的差异
    :param outputs: 输出列表
    :param names: 输出名称列表
    ...
    for i in range(num_outputs):
        for j in range(i + 1, num_outputs):
            ...
            print(f'* {name_i} vs {name_j}: {is_equal} max_abs_err {max_abs_err:.2e} max_rel_err {max_rel_err:.2e} mean_abs_err {mean_abs_err:.2e} mean_rel_err {mean_rel_err:.2e}')
            results.append([name_i, name_j, is_equal, max_abs_err, max_rel_err, mean_abs_err, mean_rel_err])
    return results
...
if __name__ == '__main__':
    ...
    # check_forward_equal_with_pytorch_float_onnx_remove_split()

    # random 100 times
    check_forward_equal_with_pytorch_float_onnx_remove_split_random()
